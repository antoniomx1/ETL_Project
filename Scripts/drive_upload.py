...
def subir_o_actualizar_archivo(nombre_archivo, ruta_archivo, carpeta_id):
    ...
    if files:
        ...
        print(f"Archivo existente {nombre_archivo} actualizado en Reports, ID: {updated_file.get('id')}")
    else:
        # Si no existe, se crea uno nuevo
        file_metadata = {'name': nombre_archivo, 'parents': [carpeta_id]}
        file = drive_service.files().create(body=file_metadata, media_body=media, fields='id').execute()
        print(f"Archivo {nombre_archivo} subido con éxito a Reports, ID: {file.get('id')}")
